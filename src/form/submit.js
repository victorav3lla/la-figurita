...
function setupValidation(form) {
  form.querySelectorAll('input, textarea, select').forEach(field => {
    field.addEventListener('blur', () => showFieldError(field, validateField(field)))
...
function setupPriceSummary(form) {
  ...
  function update() {
    ...
    document.getElementById('summary-subtotal').textContent = formatCOP(totals.subtotal)
    document.getElementById('summary-shipping').textContent = formatCOP(totals.shipping)

    const zoneLabel = SHIPPING_OPTIONS.find(o => o.id === zone)?.label || ''
    document.getElementById('summary-zone').textContent = `(${zoneLabel})`
    document.getElementById('summary-total').textContent = formatCOP(totals.total)
...
function setupPaymentButtons(form) {
  ...
  btnPayNow?.addEventListener('click', () => {
    ...
    if (paymentOrderId) paymentOrderId.textContent = `Número de pedido: ${orderId}`

    // Mostrar paso 2
    form.hidden = true
    document.getElementById('payment-step').hidden = false
    document.getElementById('payment-step').scrollIntoView({ behavior: 'smooth', block: 'start' })
  })
  ...
  btnWhatsapp?.addEventListener('click', async () => {
    ...
    btnWhatsapp.querySelector('.payment-option-title').textContent = 'Enviando...'
    ...
    try {
      ...
      btnWhatsapp.querySelector('.payment-option-title').textContent = 'Cerrar por WhatsApp'
      alert('Hubo un problema. Intenta de nuevo o escríbenos directamente por WhatsApp.')
    }
  })
...
function setupFileUpload() {
  ...
  fileInput?.addEventListener('change', () => {
    if (fileInput.files.length > 0) {
      ...
      if (sub)        sub.textContent = `${Math.round(file.size / 1024)} KB · clic para cambiar`
...
function showSuccess(method, orderId, data) {
  ...
  if (method === 'pay_now') {
    if (text) text.textContent = 'Recibimos tu comprobante. En breve confirmamos el pago y ponemos tu álbum en producción. Te escribimos por WhatsApp.'
  } else {
    // WhatsApp: mostrar botón para abrir chat
    const batch  = getBatch(data.batch_id)
    const waText = encodeURIComponent(
      `Hola! Acabo de hacer un pedido en lafigurita.com.\n` +
